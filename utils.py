...
def embedding_indexing_system(dataset, model):
    embeddings = np.zeros((len(dataset), 30))
    idx_to_image = []
    ...
    with torch.no_grad():
        ...
        for data_list in dataset:
            embeddings[k] = model.get_embedding(data_list[1].unsqueeze(0).cuda()).data.cpu().numpy()
            idx_to_image.append((data_list[0],data_list[2]))
            k += 1

    return embeddings, idx_to_image
...
def extract_embeddings(dataloader, model):
    with torch.no_grad():
        model.eval()
        embeddings = np.zeros((len(dataloader.dataset), 30))
